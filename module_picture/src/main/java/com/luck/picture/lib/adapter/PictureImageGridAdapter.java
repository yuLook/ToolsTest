...
public class PictureImageGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private void changeCheckboxState(ViewHolder contentHolder, LocalMedia image) {
        ...
        if (config.maxFileSize>0){
            ...
            try {
                fis = new FileInputStream(image.getRealPath());
                long size = fis.available();
                if (size>config.maxFileSize*1048576){
                    showPromptDialog("文件大小超过限制，请重新选择合适的文件");
                    return;
                }
            }catch (Exception e){
            }finally {
        ...
        if (PictureMimeType.isHasImage(image.getMimeType())){
            ...
            if (config.maxImageScale>0){
                if (image.getWidth()>5000 || image.getHeight()>5000){
                    showPromptDialog("文件宽高超过限制，请重新选择合适的文件");
                    return;
                }

                Float width = Float.parseFloat(image.getWidth()+"");
                Float height = Float.parseFloat(image.getHeight()+"");
                if (width/height>6 || height/width>6){
                    showPromptDialog("文件尺寸比例不符合要求，请重新选择合适的文件");
                    return;
                }
            }
